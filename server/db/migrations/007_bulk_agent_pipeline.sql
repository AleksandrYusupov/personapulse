...
as $$
  ...
  select
    ...
    coalesce(
      (
        select jsonb_agg(to_jsonb(recent) order by recent.created_at asc)
        from (
          select m.*
...
begin
  ...
  values (
    ...
    v_event.id, 1, case when v_is_active then 'scheduled' else 'paused_inactive' end,
    p_pause_seconds,
...
begin
  ...
  values (
    p_mood_job_id, 'mood_detection', p_event_id, p_conversation_id,
    'mood_detection:' || p_event_id::text, 'analysis', 10
  )
  on conflict (idempotency_key) do nothing;

  perform pg_notify('personapulse_jobs_analysis', p_event_id::text);
